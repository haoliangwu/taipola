...
Issues and specs for this repo live as markdown files in `.scratch/`. The directory is
gitignored and absent from history, so the tracker is never published: these files ARE
the tracker.
