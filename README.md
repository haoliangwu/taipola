...
- **表格各列等宽**（`grid-auto-columns: minmax(0, 1fr)`），不做按内容自适应；样式对齐 Typora：单元格 1px 网格线 + 外框、表头行浅灰底加粗。`| --- |` 行在渲染态**不占高度**（那个位置由表头的下边线承担），DOM 里保留一份隐藏的源码副本，供从 DOM 重建文档时还原。
- **空行按源码行数渲染**：空源行与折叠的围栏行各占一个空行盒；连续空行不再合并成一个盒子。
- **标题前缀按块显现**：光标在标题行时 `# ` 显示为暗色；不在该块时折叠。
- 图片渲染为真实 `<img>`（`max-width: 100%`，按原始尺寸缩放），加载完成后会引起一次布局位移；光标进入图片构造时折叠回 `![]()` 源码。
- mermaid、数学公式尚未支持。
