...
快捷键：`Cmd/Ctrl + B/I/K/E`（粗体/斜体/链接/行内代码）、`Cmd/Ctrl + 1..6`（标题级别，重复按同级别可取消）、`Cmd/Ctrl + Shift + K`（删除整行）、`Tab` / `Shift+Tab`（列表项缩进 / 退回一层）、`Cmd/Ctrl + O/S`（打开/保存）、`Cmd/Ctrl + Shift + S`（另存为）、`Cmd/Ctrl + Shift + \`（切换大纲）、`Cmd/Ctrl + N`（新建）、`Esc`（取消焦点）。
...
- **段落内的软换行渲染成一个空格**，与导出一致（`breaks: false`；Markdown 里段落内的单换行就是空格）。在此之前视图在这里硬断行，于是同一份文档在屏幕上和导出文件里是两种排版——英文段落按 80 列手工折行时最刺眼，一句话看起来被拆成几句。做法：软换行的两端都改成 `inline`（`vl-soft` / `vl-continues`），它们合成一个匿名块，由列宽决定折行；那个换行本身由 `::after { content: ' ' }` 补回，伪元素不进 DOM，所以从 DOM 重建源码时换行还在。**行盒仍按源码行切分**——光标的定位算术按源码行建索引、按行累加基线，这条不变。
